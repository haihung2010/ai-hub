...
_RATES: list[tuple[tuple[str, str], tuple[float, float]]] = [
    ...
    (("*", "*"), (0.0, 0.0)),                     # default: no cost
]


def _match_rate(provider: str, model: str) -> tuple[float, float]:
    ...
    model string. A catch-all ``("*", "*")`` always matches last.
    """
    p = (provider or "").lower()
    m = (model or "").lower()
...
def calculate_cost_usd(
    ...
    in_rate, out_rate = _match_rate(provider, model)
